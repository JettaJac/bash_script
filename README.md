...
Учебный проект по мониторингу и исследованию состояния системы в реальном времени.

##
## Contents

1. [Chapter I - Краткая информация](#chapter-i) \
    1.1. [GoAccess](#goaccess) \
    1.2. [Prometheus](#prometheus) \
    1.3. [Grafana](#grafana)
2. [Chapter II - Требования и задания](#chapter-ii) \
...
## Chapter II - Тербования и задания

- Написанные Bash-скрипты должны находиться в текущей папке
...
Написать bash-скрипт, который создает определенное количество папок и файлов по определенному пути. Скрипт запускается с 6 параметрами. Пример запуска скрипта: \
...
Написать bash-скрипт, который создает файлы и папки с определенными параметрами. Скрипт запускается с 3 параметрами. Пример запуска скрипта: \
...
Скрипт должн формировать html страничку по формату **Prometheus**, которую будет отдавать **nginx**. \
